<script lang="ts">
	...
	let manifest: RotationManifest | "not_found" | undefined = undefined;
	...
	$: {
		...
		if (loaded) {
			...
			fetch(fetch_string).then((value: Response) => {
				if (value.ok) {
					value.json().then((res: RotationManifest) => {
						manifest = res;
					});
				} else {
	...
	$: {
		...
		if (selected_rotation && selected_rotation.responsibilities) {
			for (const responsibility of selected_rotation.responsibilities) {
				for (const day of responsibility.days) {
					...
					if (di === undefined) {
						console.error("Unexpected short day", day);
					} else {
		...
		responsible_days.forEach((v) => {
			responsible_days_arr.push(v);
		});
		...
		<div class="manifest">
			<div class="title">
				...
				<div class="columns margins">
					<div>
						<Select key={dowfunc} bind:value={dow}>
			...
			<div class="manifest">
				{#if selected_rotation !== undefined}
					...
					<RotationDisplay
						...
						{dow}
						hide_wetreads={!show_wetreads}
						hide_volumes={!show_volumes}
					/>
					{#if empty_day}
						<div class="warning warning_empty">
							<div class="warning_empty_desc">
								Warning: This rotation has no responsibilities on {shortdow}. Try:
							</div>
