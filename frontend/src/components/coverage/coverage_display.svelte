<script lang="ts">
	...
	onMount(() => {
		fetch("data/active_coverage_tree" + key + ".json").then((value: Response) => {
			if (value.ok) {
				value.json().then((res: SiteTree) => {
					site_tree = res;
		...
		fetch("data/exam_categories" + key + ".json").then((value: Response) => {
			if (value.ok) {
				value.json().then((res: ExamCategory[]) => {
					exam_categories = res;
	...
	let getExamLabel = (option: ExamCategory) => {
		...
		return option.exam_description;
	};
				...
				<Content>
					<div class="button_container">
						<FormField>
							...
							<Switch
								color="primary"
								disabled={exam_categories === undefined}
								bind:checked={search_by_exam_description}
							/>
						</FormField>
					</div>
					<div class="button_container">
						<Select label="Site" bind:value={active_coords.site}>
					...
					<div class="button_container">
						<Select label="Context" bind:value={active_coords.context}>
					...
					<div class="button_container">
						{#if search_by_exam_description && exam_categories !== undefined}
							...
							/>
						{:else}
							<Select label="Exam" bind:value={active_coords.exam}>
                    ...
                    -->
					<div class="button_container">
						<Select label="Day of the week" key={dowfunc} bind:value={active_coords.dow}>
