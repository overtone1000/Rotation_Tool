...
use crate::{analysis::analysis_datum::{AnalysisDatum, ComparisonDatum}, coverage::{coordinate::CoverageCoordinates, coverage_and_work_day::CoverageAndWorkDay, units::CoverageUnit, work_coverage_map::maps::CoverageMap}, serialization::output::JSONFileOut};

fn get_by_rotation_average(
    coverage_map: &CoverageMap,
) -> BTreeMap<String, ComparisonDatum> {
    ...
    for (rotation, datum) in &mut retval
    {
        ...
        let scale_factor:f64=1.0/(f64::try_from(dates.len() as u32).expect("Invalid denominator"));
...
pub fn compare(
    active_map:&CoverageMap,
    proposed_map:&CoverageMap
) -> ComparisonResult {
