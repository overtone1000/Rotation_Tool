...
impl Table for Location_Categories {
    type Entry = LocationCategoryEntry;
    ...
    fn build_from_headers_and_row(
        ...
        row: &Vec<String>,
    ) -> Result<LocationCategoryEntry, Box<dyn std::error::Error>> {
        Ok(LocationCategoryEntry {
...
impl Location_Categories {
    pub fn create(filename: &str) -> Location_Categories {
        Location_Categories {
